...
RewriteRule .* index.php [L,QSA]
      ...
      <p>В качестве обработчика может быть передана анонимная функция (делегат), имя функции или подключаемого файла.</p>
      <pre class="prettyprint linenums">
function foo() {
	...
}
...
PIE::route('/.*/', function(){
	PIE::reroute('/home');
     ...
     <section id='template'>
      <h1>Шаблоны</h1>
      <p>Всем известно, что логику необходимо отделять от представления. И <code>{PIE::*}</code> с этим согласен, по этому у него есть простенький и быстрый <code>template</code>. Для его использования сначала следует настроить шаблонизатор, указав каталог, в котором будут расположены файлы шаблонов.</p>
      <pre class="prettyprint linenums">
